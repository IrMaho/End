...
let endStatusBarRunItem;
let endStatusBarTestItem;
let endDiagnosticManager;
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider({ language: 'end', scheme: 'file' }, new EndCompletionItemProvider(), '.', ':', '@', '>', ' ', '"'));
    ...
    context.subscriptions.push(vscode.languages.registerDefinitionProvider({ language: 'end', scheme: 'file' }, new EndDefinitionProvider()));
    ...
    vscode.commands.registerCommand('end.runTest', (fnName) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const file = editor.document.fileName;
    ...
    // Bench 1M Ops
    vscode.commands.registerCommand('end.bench1MOps', (fnName) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const file = editor.document.fileName;
...
function deactivate() {
    if (endStatusBarRunItem)
        endStatusBarRunItem.dispose();
    if (endStatusBarTestItem)
        endStatusBarTestItem.dispose();
}
function getCompilerExecutable() {
    const config = vscode.workspace.getConfiguration('end');
    const userPath = config.get('compilerPath');
    if (userPath && userPath !== 'endc')
        return userPath;
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '';
...
class EndDiagnosticManager {
    ...
    validateDocument(document, showPopup = false) {
        const exe = getCompilerExecutable();
...
// -----------------------------------------------------------------------------
class EndCompletionItemProvider {
    provideCompletionItems(document, position, token, context) {
        ...
        while ((match = symbolRegex.exec(docText)) !== null) {
            ...
            else if (kindStr === 'feature' || kindStr === 'hub' || kindStr === 'agent')
...
class EndTreeItem extends vscode.TreeItem {
    ...
    descriptionText;
    collapsibleState;
    constructor(label, descriptionText, collapsibleState) {
